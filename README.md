...
- [clone-react-dnd](#clone-react-dnd)
  - [애플리케이션 소개](#애플리케이션-소개)
  - [목차](#목차)
  - [기획 배경 및 상세 소개](#기획-배경-및-상세-소개)
  - [프로젝트 구조](#프로젝트-구조)
  - [사용 방법](#사용-방법)
    - [useDragClone](#usedragclone)
    - [useDropClone](#usedropclone)
    - [useGlobalStates](#useglobalstates)
    - [useTouchDnd](#usetouchdnd)
  - [기술 스택](#기술-스택)
  - [기술적 고민](#기술적-고민)
    - [해결한 문제](#해결한-문제)
    ...
    - **반환 배열**
      ...
      - **[2]**(`setSettings`): 드래그 앤 드롭 구현에 사용할 수 있는 메서드를 포함하는 객체입니다.
        ...
        - `makeDraggable(param: boolean)`: 드래그 속성이 토글 형식으로 부여되도록 설정하는 메서드입니다.
          - 드래그 속성이 필요 없는 경우 `false`를, 드래그 속성이 필요한 경우 `true`를 값으로 받습니다.
...
- **상세 설명**
  ...
  - `isDropped`: 드롭 이벤트가 정상적으로 발생했는지를 나타냅니다.

[목차](#목차)

### useTouchDnd

- `useTouchDnd`는 모바일 화면에서 터치 이벤트로 드래그 앤 드롭 기능을 구현해야 할 때 사용하는 hook입니다.
- **상세 설명**
  ...
  - **파라미터**
    - `touchStyleOptions`: 터치 이벤트를 적용하기에 앞서 부여할 옵션입니다. 드래그 대상을 드래그할 때 표시할 미리보기, 드래그 대상을 드롭할 자리 강조 등의 효과를 부여할 수 있습니다.
      - `cloneTgtStyle`: 터치로 이동시킬 대상의 미리보기 스타일을 설정합니다. 대상의 width, height, position, left, top, opacity, background 전부 혹은 일부를 수정할 수 있습니다.
      - `highlightStyles`: 터치 후 드래그하는 동안 대상이 드롭될 자리를 강조하는 효과를 부여할 수 있습니다. 대상의 transition, currEleBoxShadow(= 현재 드래그 대상이 드롭될 자리 강조), dropTgtBoxShadow(= 드래그로 목록의 끝에 도달했을 때 마지막 요소의 강조 효과), exceptionBoxShadow(= 드롭될 자리 이외의 요소에서 강조 효과 제거 혹은 별도의 강조 효과 부여) 전부 혹은 일부를 수정할 수 있습니다.
  - **반환 배열**
    - **[0]**(`makeTouchTgtClone`): touchstart 시점에 드래그할 대상의 미리보기 요소를 생성하는 함수입니다.
    ...
    - **[3]**(`detectDropEvt`): touchend 시점에 드래그 중이던 요소를 드롭하고, 이에 맞춰 요소 목록의 순서를 변경하는 함수입니다.
      - 인자로 드롭 대상, 드롭 결과를 문자열 형태로 만들어 기존 상태 값과 대체할 함수를 받습니다.
...
- 프로젝트 이름은 clone-react-dnd이지만, 실제로는 react-dnd 라이브러리의 리버스 엔지니어링에 가까운 프로젝트였습니다.
  - react-dnd 공식 문서의 [예제](https://react-dnd.github.io/react-dnd/examples)만 참고하여 어떻게 동작하는지 분석한 후 기능을 구현했기 때문입니다.
- 이런 방식을 선택한 이유는 다음과 같습니다.
    1. 익숙하지 않은 api에 적응
        - 이전에 들었던 강의를 통해 Drag event api를 사용해본 경험은 있지만 사용 범위가 제한적이라 기능 자체에 익숙해지진 않은 상태였습니다.
        - 실제 활용 사례를 참조하는 것을 통해 api를 어떻게 사용할 수 있는지 간접 경험하여, 혼자서 api를 분석하는 것보다 빠르게 적응하는 데 도움이 될 것으로 판단했습니다.
    2. 시간 절약
        - 처음부터 어떤 기능을 구현할지 기획하고 작업하는 것보다, 이미 구현된 사례를 분석해 필요한 기능만 구현하는 편이 시간이 절약될 것으로 판단했기 때문입니다.
- 이번 프로젝트를 통해 당초 목표로 했던 3가지 경험을 쌓을 수 있었습니다.
  - Drag event api 사용 경험 쌓기
    - 추후 드래그 앤 드롭 관련 상호작용을 구현할 때 도움이 될 것으로 기대됩니다.
    - 아쉬운 점은 위 ‘해결이 필요한 문제’에 작성한 것처럼 사용자 역할이 너무 과도하게 구현된 것 같다는 점입니다. 추후 각 Custom Hook을 좀 더 간단하게 구현해 사용자들이 간편하게 사용할 수 있게 하는 것이 필요해 보입니다.
  - Custom Hook 구현 경험 쌓기
    - Custom Hook을 직접 구현해보기 전에는 막연히 어렵다고만 느껴졌는데, 실제로 구현해보니 지레 겁을 먹었던 것 같다는 생각이 들었습니다.
    - 이제까지 작업했던 내용들이나 앞으로 작업할 프로젝트에서 Custom Hook을 사용해 로직 분리가 가능할 것으로 기대하고 있습니다.
  - 패키지 매니저로 설치할 수 있는 라이브러리 제작 경험 쌓기
    - 패키지 매니저로 설치할 수 있는 형태로 만드는 것은 [yalc 라이브러리](https://www.npmjs.com/package/yalc)를 이용했습니다.
    - 앞으로 실제로 라이브러리를 만드는 일이 있을지 모르겠지만, 프로젝트에서 로직을 분리하는데 이번 경험을 사용할 수 있을 것 같습니다.
...
```bash
# 현재 디렉터리에 클론하는 경우
$ git clone https://github.com/godcl1623/CLONE-react-dnd.git .

# 하위 폴더에 클론하는 경우
$ git clone https://github.com/godcl1623/CLONE-react-dnd.git ./clone-react-dnd
```
...
- 배포용 빌드 적용 방법
  - clone-dnd 폴더를 src 디렉터리 내부로 복사합니다.
  - 자세한 사용 방법은 사용 방법 항목을 참조해주세요.
